...
- **HTTP REST uploads with live progress feedback** — keep uploads simple and transparent in the web app.
...
| **Upload progress UX (XHR `onprogress`)** | Immediate user feedback with minimal backend complexity |

### Tech stack

- **Runtime:** Bun ≥ 1.3
- **Backend:** TypeScript, Elysia, `bun:sqlite` via Drizzle ORM (schema in TS, migrations generated by drizzle-kit), better-auth (email/password + cookie sessions)
...
- [x] Simple multipart upload with web-side progress feedback (`XMLHttpRequest.upload.onprogress`)
...
### Phase 3 — Upload UX + progress (Week 5)

**Deliverable:** Clear, reliable upload feedback in the browser.
